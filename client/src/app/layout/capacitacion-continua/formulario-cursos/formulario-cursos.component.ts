...
export class FormularioCursosComponent implements OnInit {
    @ViewChild('encabezadoFormularioCursos') encabezadoFormularioCursos: ElementRef;
    @ViewChild('cuerpoFormularioCursosPG1') cuerpoFormularioCursosPG1: ElementRef;
    @ViewChild('cuerpoFormularioCursosPG2') cuerpoFormularioCursosPG2: ElementRef;
    @ViewChild('pieFormularioCursosPG1') pieFormularioCursosPG1: ElementRef;
    @ViewChild('pieFormularioCursosPG2') pieFormularioCursosPG2: ElementRef;
    busy: Promise<any>;
    ...
    imprimir(): void {
        html2canvas(this.encabezadoFormularioCursos.nativeElement).then(canvasEncabezado => {
            const encabezadoImg = canvasEncabezado.toDataURL('image/png');
            html2canvas(this.cuerpoFormularioCursosPG1.nativeElement).then(canvasCuerpoPG1 => {
                const PG1Img = canvasCuerpoPG1.toDataURL('image/png');
                html2canvas(this.cuerpoFormularioCursosPG2.nativeElement).then(canvasCuerpoPG2 => {
                    const PG2Img = canvasCuerpoPG2.toDataURL('image/png');
                    html2canvas(this.pieFormularioCursosPG1.nativeElement).then(canvasPiePG1 => {
                        const piePG1Img = canvasPiePG1.toDataURL('image/png');
                        html2canvas(this.pieFormularioCursosPG2.nativeElement).then(canvasPiePG2 => {
                            ...
                            doc.save('FormularioCursos_' + this.personaLogeada.identificacion + '.pdf');
                        });
                    });
                });
            });
