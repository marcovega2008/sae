...
@Component({
   selector: 'app-horariocapacitacioncontinua',
   templateUrl: './horariocapacitacioncontinua.component.html',
   styleUrls: ['./horariocapacitacioncontinua.component.scss']
})

export class HorarioCapacitacionContinuaComponent implements OnInit {

   busy: Promise<any>;
   entidades: HorarioCapacitacionContinua[];
   entidadSeleccionada: HorarioCapacitacionContinua;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: HorarioCapacitacionContinuaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: HorarioCapacitacionContinua): boolean {
      return true;
   }
   ...
   crearEntidad(): HorarioCapacitacionContinua {
      const nuevoHorarioCapacitacionContinua = new HorarioCapacitacionContinua();
      nuevoHorarioCapacitacionContinua.id = 0;
      return nuevoHorarioCapacitacionContinua;
   }

   add(entidadNueva: HorarioCapacitacionContinua): void {
   ...
   update(entidadParaActualizar: HorarioCapacitacionContinua): void {
   ...
   delete(entidadParaBorrar: HorarioCapacitacionContinua): void {
   ...
   refresh(): void {
      ...
      this.entidades = HorarioCapacitacionContinua[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: HorarioCapacitacionContinua): void {
