...
@Component({
   selector: 'app-cursocapacitacioncontinua',
   templateUrl: './cursocapacitacioncontinua.component.html',
   styleUrls: ['./cursocapacitacioncontinua.component.scss']
})

export class CursoCapacitacionContinuaComponent implements OnInit {

   busy: Promise<any>;
   entidades: CursoCapacitacionContinua[];
   entidadSeleccionada: CursoCapacitacionContinua;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CursoCapacitacionContinuaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CursoCapacitacionContinua): boolean {
      return true;
   }
   ...
   crearEntidad(): CursoCapacitacionContinua {
      const nuevoCursoCapacitacionContinua = new CursoCapacitacionContinua();
      nuevoCursoCapacitacionContinua.id = 0;
      return nuevoCursoCapacitacionContinua;
   }

   add(entidadNueva: CursoCapacitacionContinua): void {
   ...
   update(entidadParaActualizar: CursoCapacitacionContinua): void {
   ...
   delete(entidadParaBorrar: CursoCapacitacionContinua): void {
   ...
   refresh(): void {
      ...
      this.entidades = CursoCapacitacionContinua[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CursoCapacitacionContinua): void {
